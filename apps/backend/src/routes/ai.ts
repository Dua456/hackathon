...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      logger.info({ userId: req.userId }, 'Audio analysis requested');
      ...
      logger.error({ error, userId: req.userId }, 'Audio analysis failed');
      res.status(500).json({
        success: false,
        error: {
...
router.post('/test-analysis', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    logger.info({ userId: req.userId }, 'Test analysis requested');
